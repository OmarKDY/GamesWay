function CGame(oData){
    ...
    this._init = function(){
        ...
        _oContainerNewPieces = new createjs.Container();
        s_oStage.addChild(_oContainerNewPieces);
    ...
    this._createForm = function (){
        for(var i = 0; i<3; i++){
            ...
            _aForm.push(new CForms(i, iValue, _oContainerNewPieces));
            _iFormsOn++;    
    ...
    this.dragForm = function(event, container, i){
        if(_aDrag[i] && _aForm[i] ){
           _aForm[i].setPos( (event.stageX/s_iScaleFactor) - _iMouseOffset.x , (event.stageY/s_iScaleFactor) - _iMouseOffset.y-150, container);
        }     
    };
