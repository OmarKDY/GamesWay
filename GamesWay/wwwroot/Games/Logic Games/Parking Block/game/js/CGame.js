function CGame(oData, iLevel, iScore){
    ...
    this._init = function(){
        ...
        _oGridContainer.regX = NUM_COLS*CELL_WIDTH/2;
        _oGridContainer.regY = NUM_ROWS*CELL_HEIGHT/2;
        s_oStage.addChild(_oGridContainer);
        
        /*
        var oDebugShape = new createjs.Shape();
        oDebugShape.graphics.beginFill("rgba(255,255,255,0.41)").drawRect(START_X_GRID, START_Y_GRID, NUM_COLS*CELL_WIDTH, NUM_ROWS*CELL_HEIGHT);
        _oGridContainer.addChild(oDebugShape);
        */
        
        for(_iBlocksPlaced=0; _iBlocksPlaced < _aLevelInfo.length; _iBlocksPlaced++){
            var oBlock = new CBlocks(_aLevelInfo[_iBlocksPlaced].obj_x, _aLevelInfo[_iBlocksPlaced].obj_y, _aLevelInfo[_iBlocksPlaced].obj_type, _iBlocksPlaced, _oGridContainer);
            
            _aBlock.push( oBlock );
            this._updateGrid(_aLevelInfo[_iBlocksPlaced].obj_x, _aLevelInfo[_iBlocksPlaced].obj_y, _aLevelInfo[_iBlocksPlaced].obj_type);
            
            if(_aLevelInfo[_iBlocksPlaced].obj_type === OBJ_TYPE_PLAYER){
                _oPlayer = oBlock;
            }
        }
    ...
    this.onRestart = function(){
        ...
        for(var i=0; i < _iBlocksPlaced; i++){
            if(_aLevelInfo[i].obj_type === OBJ_VERTICAL_2 || _aLevelInfo[i].obj_type === OBJ_VERTICAL_3){
